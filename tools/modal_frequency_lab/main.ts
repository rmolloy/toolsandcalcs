...
function estimatePeakFreq() {
  ...
  const values = spectrum.dbs?.length ? spectrum.dbs : spectrum.mags;
  let maxIdx = 0;
  let maxVal = -Infinity;
...
async function handleRingdownAnalyze() {
  const el = document.getElementById("ringdown_results");
  if (el) el.innerHTML = '<div class="small muted">Analyzing…</div>';
  try {
    const sampleInput = document.getElementById("sample_length");
    const sampleLengthMs = Number(sampleInput?.value) || 1500;
    ...
    if (!slice) throw new Error("No audio loaded.");
    ...
    if (el) el.innerHTML = `<div class="small muted">Ring-down analysis failed: ${err.message || err}</div>`;
    drawRingdownEnvelope([]);
  }
}
...
function bindControls() {
  ...
  if (btnRingdown) btnRingdown.addEventListener("click", () => {
  ...
  if (btnClearRingdown) btnClearRingdown.addEventListener("click", clearRingdown);
}
