...
import {
  braceStockLongModeConfirmationApply,
  braceStockLongModeConfirmationClear,
} from "./resonate_brace_stock_confirmation.js";
import { measureModeNormalize, peakAnalysisSourceMeasureModeResolve } from "./resonate_mode_config.js";

export type BraceStockEstimatePresentation = {
  status: "incomplete" | "needs-confirmation" | "ready";
  summary: string;
  metrics: Array<{ label: string; value: string }>;
  primaryAction: "set-measurements" | "confirm-long" | "open-details";
  primaryLabel: string;
  replaceLongLabel?: string;
  clearLongAvailable?: boolean;
  transferAvailable: boolean;
};

type BraceStockEstimatePanelDependencies = {
  selectedPeakResolve: () => { key: string; freq: number | null } | null;
  render: () => void;
  transfer?: () => void;
};

export function braceStockEstimatePresentationBuild(
  state: Record<string, any>,
  selectedPeak?: { key: string; freq: number | null } | null,
): BraceStockEstimatePresentation {
...
export function braceStockEstimatePanelRenderFromState(
  state: Record<string, any>,
  selectedPeak?: { key: string; freq: number | null } | null,
) {
  ...
  content.innerHTML = braceStockEstimateMarkupBuild(
    braceStockEstimatePresentationBuild(state, selectedPeak),
  );
}

export function braceStockEstimatePanelInitialize(
  state: Record<string, any>,
  dependencies: BraceStockEstimatePanelDependencies,
) {
  const panel = document.getElementById("brace_stock_estimate") as (HTMLElement & {
    __braceStockEstimateListenerAttached?: boolean;
  }) | null;
  if (!panel || panel.__braceStockEstimateListenerAttached) return;
  panel.__braceStockEstimateListenerAttached = true;
  panel.addEventListener("click", (event) => {
    const action = (event.target as HTMLElement | null)?.closest<HTMLElement>("[data-brace-stock-action]")?.dataset.braceStockAction;
    if (!action) return;
    void braceStockEstimateActionApply(state, action, dependencies);
  });
}

async function braceStockEstimateActionApply(
  state: Record<string, any>,
  action: string,
  dependencies: BraceStockEstimatePanelDependencies,
) {
  ...
  if (action === "transfer") dependencies.transfer?.();
}

function braceStockEstimateMarkupBuild(presentation: BraceStockEstimatePresentation) {
