...
function peakAnalysisRingdownRender(data: PeakAnalysisRingdownData | null) {
  ...
  if (provenance) provenance.textContent = peakAnalysisProvenanceTextBuild(data.provenance, data.result);
...
function peakAnalysisProvenanceTextBuild(
  provenance: PeakAnalysisRingdownProvenance,
  result: PeakAnalysisRingdownResult,
) {
  ...
  const spectralBandwidth = Number.isFinite(provenance.bandwidthHz)
    ? `spectral BW ${(provenance.bandwidthHz as number).toFixed(2)} Hz`
    : "automatic spectral BW";
  const isolationBandwidth = Number.isFinite(result.isolationBandwidthHz)
    ? `isolation ${(result.isolationBandwidthHz as number).toFixed(2)} Hz`
    : null;
...
function peakAnalysisRingdownFitProjectionAxisBuild(args: { observed: number[]; slope: number; startAmplitude: number }) {
  ...
  if (!Number.isFinite(projectionEnd) || projectionEnd <= lastObserved) return args.observed;
  const extensionCount = 120;
  const extension = Array.from(
    { length: extensionCount },
    (_value, index) => lastObserved + ((projectionEnd - lastObserved) * (index + 1)) / extensionCount,
  );
...
function peakAnalysisRingdownLayoutBuild(data: { x: number[]; response: number[]; fit: { x: number[]; y: number[] } | null }, result: PeakAnalysisRingdownResult) {
  return {
...
function peakAnalysisRingdownXAxisRangeBuild(x: number[]) {
  const first = x[0];
