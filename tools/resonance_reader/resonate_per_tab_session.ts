...
export function resonancePerTabSessionCreate(): ResonancePerTabSession | null {
...
function resonancePerTabPointerSessionRead() {
  const api = (window as any).PerTabToolSession;
...
function resonancePerTabSnapshotIdResolve(pointerSession: {
  read(): { snapshotId?: unknown } | null;
  write(state: { snapshotId: string }): boolean;
}) {
...
function resonancePerTabSnapshotBuild(state: Record<string, any>) {
  return Object.fromEntries(STATE_KEYS.map((key) => [key, state[key]]));
}

function resonancePerTabSnapshotApplyToState(state: Record<string, any>, snapshot: Record<string, any>) {
...
async function resonancePerTabSnapshotRead(snapshotId: string): Promise<Record<string, any> | null> {
...
async function resonancePerTabSnapshotWrite(snapshotId: string, state: Record<string, any>): Promise<void> {
  const database = await resonancePerTabDatabaseOpen();
  await resonancePerTabRequestRead(
    database.transaction(STORE_NAME, "readwrite").objectStore(STORE_NAME).put({ version: SESSION_VERSION, state }, snapshotId),
  );
  database.close();
}

function resonancePerTabDatabaseOpen(): Promise<IDBDatabase> {
...
function resonancePerTabRequestRead<T>(request: IDBRequest<T>): Promise<T> {
  return new Promise((resolve, reject) => {
    request.onsuccess = () => resolve(request.result);
