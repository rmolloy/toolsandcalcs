...
function localPeakIndexesResolveInBand(freqs: number[], mags: number[], minHz: number, maxHz: number) {
  ...
  for (let i = 0; i < freqs.length; i += 1) {
    ...
    if (v >= prev && v >= next) peakIndexes.push(i);
  }
  return peakIndexes;
}

function nearestIndexResolveForFrequency(freqs: number[], targetHz: number) {
  let bestIndex = -1;
...
function harmonicWeightResolve(index: number) {
  if (index === 1) return 0.35;
  return 1 / Math.pow(index, NOTE_F0_HARMONIC_DECAY_BASE);
}

function harmonicMagnitudeResolve(freqs: number[], mags: number[], frequencyHz: number) {
...
function harmonicScoreResolve(freqs: number[], mags: number[], fundamentalHz: number) {
  let score = 0;
...
function subharmonicCandidatesResolveFromPeakFrequency(peakFrequencyHz: number, minHz: number, maxHz: number) {
  const candidates: number[] = [];
  NOTE_F0_SUBHARMONIC_DIVISORS.forEach((divisor) => {
    const candidate = peakFrequencyHz / divisor;
    if (!Number.isFinite(candidate) || candidate < minHz || candidate > maxHz) return;
    candidates.push(candidate);
  });
  return candidates;
}

function candidateFrequenciesResolveFromPeakIndexes(
  peakIndexes: number[],
  freqs: number[],
  minHz: number,
  maxHz: number,
  ...
  peakIndexes.forEach((peakIndex) => {
    ...
    if (!Number.isFinite(peakFrequency)) return;
...
export function fundamentalFrequencyResolveFromSpectrum(
  freqs: number[],
  mags: number[],
  minHz: number,
  maxHz: number,
  ...
  if (!peakIndexes.length) return null;
...
function f0HpsCandidatesResolveFromSpectrum(
  freqs: number[],
  mags: number[],
  hpsHarmonics: number,
) {
...
function peakIndexResolveFromSeries(series: number[]) {
  let bestIndex = 0;
...
function peakIndexParabolicRefine(series: number[], peakIndex: number) {
  if (peakIndex <= 0 || peakIndex >= series.length - 1) return peakIndex;
  const left = series[peakIndex - 1];
  const mid = series[peakIndex];
  const right = series[peakIndex + 1];
  const denominator = left - (2 * mid) + right;
  if (Math.abs(denominator) <= 1e-9) return peakIndex;
  const delta = 0.5 * (left - right) / denominator;
  return peakIndex + Math.max(-1.5, Math.min(1.5, delta));
}

function f0HarmonicRefineFromCandidate(
  candidateHz: number,
  freqs: number[],
  mags: number[],
  settings: { hpsHarmonics: number; f0Min: number; f0Max: number },
) {
  if (!Number.isFinite(candidateHz)) return null;
  ...
  for (let divisor = 1; divisor <= maxDivisor; divisor += 1) {
    ...
    if (!Number.isFinite(frequency) || frequency < settings.f0Min || frequency > settings.f0Max) continue;
...
export function fundamentalFrequencyHpsResolveFromSpectrum(
  freqs: number[],
  mags: number[],
  settings = NOTE_F0_HPS_SETTINGS,
) {
  ...
  const peakFrequency = Number.isFinite(binWidth)
    ? (freqs[peakIndex] + (refinedIndex - peakIndex) * (binWidth as number))
    : freqs[peakIndex];
  return f0HarmonicRefineFromCandidate(peakFrequency, freqs, mags, settings);
}

function octaveCorrectedFrequencyResolve(
  dominantFrequency: number | null,
  hpsFrequency: number | null,
) {
  if (Number.isFinite(dominantFrequency) && Number.isFinite(hpsFrequency)) {
    const dominant = dominantFrequency as number;
    const hps = hpsFrequency as number;
    if (
      dominant >= NOTE_F0_OCTAVE_CORRECTION_MIN_HZ
  ...
  if (Number.isFinite(dominantFrequency)) return dominantFrequency as number;
  if (Number.isFinite(hpsFrequency)) return hpsFrequency as number;
  return null;
}
...
export async function noteSegmentationBuildFromWave(
  ...
  const notes = segmenter(wave as ArrayLike<number>, sampleRate, NOTE_SEGMENTATION_OPTIONS);
