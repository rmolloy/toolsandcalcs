...
const DEMO_PEAKS_BY_MODE: Record<"guitar" | "top" | "back", readonly number[]> = {
  guitar: [98, 178, 220],
  top: [42, 82, 126],
  back: [38, 88, 134],
};
...
export function demoWaveBuildFromMeasureMode(
  measureMode: unknown,
  sampleLengthMs = 1500,
): DemoWave {
  return demoWaveBuildFromPeaks(
    demoPeakFrequenciesResolveFromMeasureMode(measureMode),
    sampleLengthMs,
  );
}

function demoWaveBuildFromPeaks(
  peaksHz: readonly number[],
  sampleLengthMs: number,
): DemoWave {
  const sampleRate = 44100;
  const samples = Math.max(64, Math.round((sampleLengthMs / 1000) * sampleRate));
  const timeMs = new Array<number>(samples);
...
function demoWaveSampleBuildFromTime(
  t: number,
  idx: number,
  peaksHz: readonly number[],
  decayFast: number,
