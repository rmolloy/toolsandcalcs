...
import type { ResonanceTakeOverlay } from "./resonate_take_overlays.js";
...
type SpectrumPayload = {
  ...
  takeOverlays?: ResonanceTakeOverlay[];
...
export function renderSpectrum(payload: SpectrumPayload, deps: SpectrumRenderDeps) {
  ...
  bindSpectrumPlotInteractionsWhenReady(
    plot as HTMLElement,
    plotReady,
    deps,
    layout,
...
function bindSpectrumPlotInteractionsWhenReady(
  plot: HTMLElement,
  plotReady: unknown,
  deps: SpectrumRenderDeps,
  layout: any,
  modeAnnotationKeys: string[],
  modeAnnotationAnchorX: Record<string, number>,
  toneTraceIndex: number,
  mags: number[],
) {
  const bindInteractions = () => {
    resetSpectrumPlotListenerBindState(plot as any);
  ...
  if (plotReady && typeof (plotReady as Promise<unknown>).then === "function") {
...
function resetSpectrumPlotListenerBindState(plotAny: any) {
  resetModeOverrideLabelBindState();
...
function buildTakeOverlayTraces(takeOverlays: ResonanceTakeOverlay[]) {
  const lineWidth = resonanceSpectrumLineWidthResolve(MEASURED_TRACE_LINE_WIDTH);
  return takeOverlays
...
function renderSpectrumPlot(plot: HTMLElement, plotData: any[], layout: any) {
  ...
  if (plotAny.__spectrumPlotDrawing) return plotAny.__spectrumPlotReady;
...
function spectrumPlotDrainLatest(plot: HTMLElement) {
  const plotAny = plot as any;
  plotAny.__spectrumPlotDrawing = true;
  const drawNext = (): unknown => {
    ...
    const drawResult = (window as any).Plotly.newPlot(plot, latest.plotData, latest.layout, spectrumPlotConfigBuild());
...
type ToneFollowContext = { toneTraceIndex: number; mags: number[] };

function toneFollowContextRead(plot: HTMLElement): ToneFollowContext {
  return (plot as any).__toneFollowContext ?? { toneTraceIndex: -1, mags: [] };
}
...
function bindFftToneHoverFollow(plot: HTMLElement, toneTraceIndex: number, mags: number[]) {
  ...
  if (plotAny.__toneFollowDomBound) return;
...
function bindFftToneTouchSweep(plot: HTMLElement) {
  const sweepFromTouch = (event: TouchEvent) => {
    ...
    const context = toneFollowContextRead(plot);
    toneHoverApplyFrequency(
      toneFrequencyHzResolveFromPointer(plot, touch),
      plot,
      context.toneTraceIndex,
      context.mags,
    );
  };
...
export function toneReadoutTextBuild(freqHz: number) {
  const frequencyText = `${freqHz.toFixed(1)} Hz`;
  const out = noteAndCentsFromFreq(freqHz);
  if (!out.note || !Number.isFinite(out.cents)) return frequencyText;
  const cents = Math.round(out.cents as number);
  const sign = cents >= 0 ? "+" : "";
  return `${frequencyText} · ${out.note} ${sign}${cents}¢`;
}

function toneReadoutShow(plot: HTMLElement, freqHz: number) {
  // Plotly.newPlot empties the plot div, so the readout is recreated lazily.
  let readout = plot.querySelector<HTMLElement>(`.${TONE_READOUT_CLASS}`);
  if (!readout) {
...
function toneReadoutHide(plot: HTMLElement) {
...
export function toneFrequencyHzResolveFromPointer(plot: HTMLElement, event: { clientX: number }) {
  const fullLayout = (plot as any)?._fullLayout;
...
function modeAnnotationCompactLabelResolve(meta: ModeMeta, aliasLabel: string) {
...
function spectrumRenderModelAssemble(args: {
  ...
  const modeTraceIndexByKey = buildModeTraceIndexByKey(
    modeTraces,
    args.modes,
    takeOverlayTraces.length + overlayTraces.length + polymaxTraces.length,
    Boolean(secondaryMeasuredTrace),
    Boolean(peakHoldTrace),
  );
...
function toneSpikeTraceUpdate(plot: HTMLElement, toneTraceIndex: number, freqHz: number, mags: number[]) {
  if (!Number.isFinite(freqHz) || !toneTraceIndexIsRenderable(plot, toneTraceIndex)) return;
  const yBottom = toneSpikeFloorDbResolve(mags);
  const yTop = toneSpikeCeilingDbResolve(mags);
...
function toneSpikeTraceVisibilitySet(plot: HTMLElement, toneTraceIndex: number, visible: boolean) {
  if (!toneTraceIndexIsRenderable(plot, toneTraceIndex)) return;
  (window as any).Plotly?.restyle?.(plot, { visible }, toneTraceIndex);
}

function toneTraceIndexIsRenderable(plot: HTMLElement, toneTraceIndex: number) {
  ...
  return toneTraceIndex >= 0 && toneTraceIndex < (((plot as any).data?.length as number) ?? 0);
}
