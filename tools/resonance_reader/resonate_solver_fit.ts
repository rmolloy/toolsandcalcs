...
export function adaptParamsToSolver(raw: Record<string, number | boolean | undefined>) {
  ...
  MASS_PARAM_IDS.forEach((k) => {
    const v = out[k];
    if (typeof v === "number" && Number.isFinite(v)) out[k] = v / 1000;
  });
...
export function computeResponseSafe(params: Record<string, number | boolean | undefined>) {
  const computeResponse = (window as any).computeResponse;
  ...
  try {
    ...
    const js = (window as any).computeResponseJs;
...
export function fit4DofFromTargets(
  ...
  const evaluate = (rawParams: Record<string, any>) => {
    ...
    const err = ["air", "top", "back"].reduce((acc, k) => {
      const tgt = desired[k as "air" | "top" | "back"];
      const val = peaks[k as "air" | "top" | "back"];
