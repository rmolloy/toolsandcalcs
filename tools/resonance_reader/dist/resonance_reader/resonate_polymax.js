export function polymaxStableCandidatesFromWave(wave, sampleRate, opts = {}) {
...
export function polymaxCandidateClustersFromWave(wave, sampleRate, opts = {}) {
    return polymaxCandidateResolutionFromWave(wave, sampleRate, opts).candidates;
}
function polymaxCandidateResolutionFromWave(wave, sampleRate, opts) {
    if (!Number.isFinite(sampleRate) || sampleRate <= 0) {
        return { candidates: [], orderSweepCount: 0 };
    }
    const arr = Array.from(wave, (v) => Number(v)).filter((v) => Number.isFinite(v));
    if (arr.length < 256)
        return { candidates: [], orderSweepCount: 0 };
...
function lpcAutocorrelation(data, order) {
    ...
    for (let i = 1; i <= order; i += 1) {
        ...
        a[i] = k;
        err *= (1 - k * k);
        if (!Number.isFinite(err) || err <= 1e-12)
            return null;
    }
    return a;
}
function polesFromLpc(coeffs, sampleRate, freqMin, freqMax) {
    ...
    for (const pole of poles) {
        ...
        if (prev && Math.abs(prev.freqHz - pole.freqHz) < 0.75)
            continue;
        out.push(pole);
    }
    return out;
}
function polynomialRootsDurandKerner(coeffs) {
...
function evalPoly(coeffs, x) {
    let y = { re: coeffs[0], im: 0 };
    for (let i = 1; i < coeffs.length; i += 1)
        y = cadd(cmul(y, x), { re: coeffs[i], im: 0 });
    return y;
}
