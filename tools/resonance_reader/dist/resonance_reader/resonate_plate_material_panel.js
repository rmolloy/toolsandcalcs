...
const DEFAULT_PLATE_MATERIAL_MEASUREMENTS = {
    panelLengthMm: 555,
    panelWidthMm: 227,
    panelHeightMm: 4.1,
    panelMassG: 211.4,
...
export function plateMaterialMeasurementsResolveFromState(state) {
    plateMaterialMeasurementsSeedIntoState(state);
    return { ...state.plateMaterialMeasurements };
}
function plateMaterialMeasurementsSeedIntoState(state) {
    if (state.plateMaterialMeasurements)
        return;
    state.plateMaterialMeasurements = { ...DEFAULT_PLATE_MATERIAL_MEASUREMENTS };
}
function plateMaterialPanelBindOnce(state) {
...
function plateMaterialCardCopySyncFromState(state) {
...
function plateMaterialInputsSyncFromState(state) {
...
function plateMaterialInputEntriesBuild() {
    return Object.entries(FIELD_TO_INPUT_ID)
        .map(([fieldKey, inputId]) => {
        const input = document.getElementById(inputId);
        return input ? [fieldKey, input] : null;
    })
        .filter((entry) => entry !== null);
}
function plateMaterialCardElementGet() {
    return document.getElementById("plate_material_card");
}
function plateMaterialCardTitleElementGet() {
    return document.getElementById("plate_material_title");
}
function plateMaterialCardDescriptionElementGet() {
    return document.getElementById("plate_material_description");
}
function plateMaterialNumberParseOrDefault(raw, fieldKey) {
    const value = Number.parseFloat(raw);
    if (Number.isFinite(value) && value > 0)
        return value;
    return DEFAULT_PLATE_MATERIAL_MEASUREMENTS[fieldKey];
}
function plateMaterialNumberFormat(value) {
