...
function resonanceCaptureBlobDownload(args) {
...
function resonanceCaptureFilenameStemBuild(recordingLabel, savedAtIso) {
    const label = resonanceCaptureFilenameLabelBuild(recordingLabel);
    const timestamp = savedAtIso.replace(/[:]/g, "-").replace(/\.\d{3}Z$/, "Z");
    return `${label}-${timestamp}`;
}
function resonanceCaptureFilenameLabelBuild(recordingLabel) {
