...
export function fitModeWeightsFromDetected(modesDetected: ModeDetection[]): { air: number; top: number; back: number } {
  const byMode = new Map<string, ModeDetection>();
  modesDetected.forEach((mode) => byMode.set(mode.mode, mode));
  const weightFor = (key: "air" | "top" | "back") => {
    const prominence = Number(byMode.get(key)?.prominenceDb);
    if (!Number.isFinite(prominence)) return 1;
...
export function fitPriorsFromState(state: Record<string, any>) {
  const measuredMasses = state.fittedMassCeilings as Record<string, number> | undefined;
  const ceilings = {
