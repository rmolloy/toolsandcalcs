...
(function () {
    ...
    function fitTwoMode(envelope, dt, opts = {}) {
        ...
        const attackSkipMs = (_a = opts.attackSkipMs) !== null && _a !== void 0 ? _a : 100;
        const maxAnalysisMs = (_b = opts.maxAnalysisMs) !== null && _b !== void 0 ? _b : 2000;
        ...
        const alpha = (reg === null || reg === void 0 ? void 0 : reg.slope) ? -Math.min(reg.slope, 0) : 0;
        ...
        for (let i = detrended.length - 1; i >= 0; i -= 1) {
            if (Math.abs(detrended[i]) >= wobbleThresh) {
                wobbleEnd = i;
                break;
            }
        }
        ...
        if (wolfScore < 0.10)
            category = "None";
        else if (wolfScore < 0.25)
            category = "Mild";
        else if (wolfScore < 0.45)
            category = "Moderate";
        else if (wolfScore < 0.70)
            category = "Strong";
    ...
    if (typeof self !== "undefined" && typeof self.onmessage !== "undefined") {
        self.onmessage = (event) => {
            const { data } = event;
            if (!data || data.type !== "ANALYZE_NOTE")
                return;
            try {
                const { id, envelope, dt, attackSkipMs, maxAnalysisMs } = data;
                if (!envelope || !dt)
                    throw new Error("Missing envelope or dt");
                try {
                ...
                self.postMessage({ type: "NOTE_ERROR", id: data === null || data === void 0 ? void 0 : data.id, message: (err === null || err === void 0 ? void 0 : err.message) || String(err) });
            }
        };
    }
