...
declare function createFftEngine(opts: any): {
  magnitude: (
    wave: ArrayLike<number>,
    sampleRate: number,
    opts?: { window?: string; maxFreq?: number },
  ) => Promise<{ freqs: number[]; mags: number[]; dbs?: number[] }>;
};

type NoteResult = { f0: number | null } & Record<string, any>;
type PartialKey = "f0" | "h2" | "h3";
type PartialEntry = { key: string; label: string; freq: number };
type TapSegment = { start: number; end: number };

export const fftState = (window as any).FFTState || {};
const freqToNoteCents =
  ((window as any).FFTUtils?.freqToNoteCents as undefined | ((freqHz: number) => { name: string; centsNum: number })) ||
  undefined;
...
export const appState = {
  tapSegments: [] as TapSegment[],
  bodyModes: { air: null, top: null, back: null } as any,
  modeStatus: { air: "--", top: "--", back: "--" } as Record<string, string>,
  modeOverrides: {} as Record<string, number>,
  extraModes: [] as {
    id: string;
    label: string;
    alias?: string;
  ...
  manualSelection: null as { range: { start: number; end: number }; result: any; slice: any } | null,
  autoSelect: true,
  analysisToken: 0,
  energyMetrics: null as any,
  energySeries: null as any,
  energyMode: null as any,
  partialInstability: null as any,
};

let fftDefaultRanges: { x: [number, number]; y: [number, number] } | null = null;
...
export function setFftDefaultRanges(ranges: { x: [number, number]; y: [number, number] } | null) {
...
export function setWaveUpdatingShapes(value: boolean) {
  waveUpdatingShapes = value;
}

export function setWaveStatus(text: string) {
...
function hexToRgba(hex: string, alpha: number) {
  if (!hex || typeof hex !== "string") return `rgba(245, 175, 25, ${alpha})`;
  let h = hex.trim();
  if (h.startsWith("#")) h = h.slice(1);
  if (h.length === 3) h = h.split("").map((c) => c + c).join("");
  if (h.length !== 6) return `rgba(245, 175, 25, ${alpha})`;
  const r = parseInt(h.slice(0, 2), 16);
  const g = parseInt(h.slice(2, 4), 16);
  const b = parseInt(h.slice(4, 6), 16);
  if ([r, g, b].some((v) => Number.isNaN(v))) return `rgba(245, 175, 25, ${alpha})`;
  return `rgba(${r}, ${g}, ${b}, ${alpha})`;
}

function formatNote(freq: number | null): { name: string; cents: string } | null {
  if (!Number.isFinite(freq)) return null;
  const note = freqToNoteCents?.(freq as number);
  if (!note) return null;
...
function formatHz(val: number | null, digits = 1): string {
  if (!Number.isFinite(val)) return "--";
  return `${(val as number).toFixed(digits)} Hz`;
}

function formatCentsValue(cents: number | null) {
  if (!Number.isFinite(cents)) return "";
  const sign = (cents as number) >= 0 ? "+" : "";
  return `${sign}${Math.round(cents as number)}c`;
}

function deriveFirstThreeFrequenciesFromNote(noteResult: NoteResult | null | undefined) {
  const partials = deriveFirstThreePartialsFromNote(noteResult);
  return partials.map((p) => ({ label: p.label, freq: p.freq }));
}

function deriveFirstThreePartialsFromNote(noteResult: NoteResult | null | undefined): PartialEntry[] {
...
function detectTaps(
  ...
  sampleRate: number,
  opts: { windowMs?: number; thresholdMult?: number; minGapMs?: number; minLenMs?: number } = {},
) {
  ...
  for (let i = 0; i < wave.length; i += 1) {
    acc += Math.abs(wave[i] as number);
    if (i >= windowSamples) acc -= Math.abs(wave[i - windowSamples] as number);
...
async function averageTapSpectra(
  wave: Float32Array | number[],
  sampleRate: number,
  taps: TapSegment[],
) {
...
async function estimateF0Hps(wave: ArrayLike<number>, sampleRate: number, settings: any) {
  const targetSampleRate = sampleRate / Math.max(1, settings.hpsDownsample);
  const waveNumbers = Array.from(wave as ArrayLike<number>);
  const resampled: number[] = settings.hpsDownsample > 1
  ...
  const baseF0 = Number.isFinite(binWidth)
    ? (freqs[bestIdx] ?? refinedIdx * binWidth)
    : freqs[bestIdx];
  const f0Raw = Number.isFinite(binWidth) ? (freqs[bestIdx] + (refinedIdx - bestIdx) * binWidth) : baseF0;

  const refineByHarmonics = (f0Candidate: number) => {
    if (!Number.isFinite(f0Candidate)) return { freq: f0Candidate, score: -Infinity };
    const maxDivisor = Math.max(3, settings.hpsHarmonics * 2);
    let bestFreq = f0Candidate;
    let bestScore = -Infinity;
    const maxHarm = 6;
    const nearestMag = (target: number) => {
      let idx = 0;
    ...
    for (let div = 1; div <= maxDivisor; div += 1) {
      ...
      if (!Number.isFinite(cand) || cand < settings.f0Min || cand > settings.f0Max) continue;
      let score = 0;
  ...
  const { freq: f0 } = refineByHarmonics(f0Raw as number);
  if (!f0 || f0 < settings.f0Min || f0 > settings.f0Max) return null;
  return f0;
}

async function analyzeNoteSlice(slice: any) {
  const attackSkipMs = 40;
  const analysis = await (window as any).ModalRingdown.analyzeRingdown({
    buffer: slice.wave,
...
function pickWolfiestNote() {
  if (!appState.noteResults.length) return null;
  ...
  appState.noteResults.forEach((n) => {
    ...
    if (score > bestScore) best = n;
  });
  return best;
}

function modeFromOverrides(key: string, fallback: any) {
...
type ProximityLevel = "none" | "low" | "medium" | "high";
type ModeProximity = { level: ProximityLevel; label: string; cents: number | null };

function statusClassFromChipLabel(status: unknown): string {
  if (typeof status !== "string" || !status) return "unknown";
  return status.toLowerCase().replace(/[^a-z]+/g, "");
}

function proximityLevelFromAbsCents(absCents: number): ProximityLevel {
  if (absCents <= 7) return "high";
  if (absCents <= 15) return "medium";
  if (absCents <= 30) return "low";
  return "none";
}

function modeProximityLabel(level: ProximityLevel): string {
  if (level === "high") return "High proximity";
  if (level === "medium") return "Medium proximity";
  if (level === "low") return "Low proximity";
  return "No proximity";
}

function modeProximityTheme(level: ProximityLevel): { tint: string; border: string } {
  if (level === "high") return { tint: "rgba(240, 120, 60, 0.24)", border: "rgba(240, 120, 60, 0.45)" };
  if (level === "medium") return { tint: "rgba(245, 196, 111, 0.24)", border: "rgba(245, 196, 111, 0.45)" };
  if (level === "low") return { tint: "rgba(86, 180, 233, 0.18)", border: "rgba(86, 180, 233, 0.4)" };
  return { tint: "rgba(120, 140, 170, 0.12)", border: "rgba(120, 140, 170, 0.3)" };
}

function computeModeProximity(modeFreqHz: number | null, components: PartialEntry[]): ModeProximity {
  if (!Number.isFinite(modeFreqHz)) return { level: "none", label: "Enter mode freq", cents: null };
  if (!components.length) return { level: "none", label: "No note selected", cents: null };
  if (typeof centsBetween !== "function") return { level: "none", label: "No cents helper", cents: null };

  let bestTarget: string | null = null;
  let bestCents: number | null = null;
  let bestAbs = Infinity;
  components.forEach((comp) => {
    const cents = centsBetween(modeFreqHz as number, comp.freq);
  ...
  const centsLabel = Number.isFinite(bestCents)
    ? `${(bestCents as number) >= 0 ? "+" : ""}${Math.round(bestCents as number)}c`
    : "";
...
function renderBodyModesUi() {
  ...
  const selected = (window as any).getSelectedNoteData?.();
  const components = deriveFirstThreePartialsFromNote(selected?.result);
  const modes = allBodyModes();
  list.innerHTML = modes.map((mode) => {
    const freq = Number.isFinite(mode.peakFreq) ? (mode.peakFreq as number).toFixed(1) : "";
    ...
    const proximity = computeModeProximity(mode.peakFreq ?? null, components);
  ...
  list.querySelectorAll(".body-mode-row").forEach((row) => {
    const key = row.getAttribute("data-mode") as string;
    ...
    input.addEventListener("change", () => {
      ...
      if (isExtra) {
        ...
        if (target) target.peakFreq = Number.isFinite(val) && val > 0 ? val : null;
      } else {
      ...
      renderBodyModesUi();
      (window as any).updateForSelection?.().catch((err: any) => console.error("[Wolf] update after manual mode failed", err));
    });
  });

  list.querySelectorAll(".mode-remove").forEach((btn) => {
    btn.addEventListener("click", () => {
      const id = (btn as HTMLElement).getAttribute("data-remove");
      if (!id) return;
      appState.extraModes = appState.extraModes.filter((m) => m.id !== id);
      renderBodyModesUi();
      (window as any).updateForSelection?.().catch((err: any) => console.error("[Wolf] update after remove mode failed", err));
    });
  });
}
...
export {
  ...
  formatHz,
  formatCentsValue,
  deriveFirstThreeFrequenciesFromNote,
  ...
  modeFromOverrides,
  updateBodyModeStatus,
  statusClassFromChipLabel,
  proximityLevelFromAbsCents,
  modeProximityLabel,
  modeProximityTheme,
  computeModeProximity,
  renderBodyModesUi,
};
