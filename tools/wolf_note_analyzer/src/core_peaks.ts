import type { ModeBand, ModeBands, ModeResult, PeakCandidate, PeakRefinement, Spectrum } from "./core_types.js";
import { median } from "./core_math.js";

export function refineParabolicPeak(freqs: number[], ys: number[], idx: number): PeakRefinement | null {
...
export function analyzeModes(spectrum: Spectrum, bands: ModeBands): ModeResult[] {
  const { freqs, dbs } = spectrum;
  return Object.entries(bands).map(([key, band]) => modeResultFromBand(key, band, freqs, dbs));
}

export function estimateQFromDb(
  freqs: number[],
  dbs: number[],
...
export function modeBandWidth(freq: number): number {
  return Math.max(40, freq * 0.03);
}

export function partialBandWidth(partialKey: string, freq: number): number {
  if (partialKey === "h2" || partialKey === "h3") return Math.max(40, freq * 0.04);
  return Math.max(40, freq * 0.03);
}

export function bandOverlapRatio(f1: number, bw1: number, f2: number, bw2: number): number {
...
function collectBandPeaks(freqs: number[], dbs: number[], band: ModeBand): PeakCandidate[] {
...
function pickPeakByDbReducer(best: PeakCandidate, peak: PeakCandidate): PeakCandidate {
  return peak.db > best.db ? peak : best;
}

function pickPeakByDb(peaks: PeakCandidate[]): PeakCandidate | null {
  if (!peaks.length) return null;
  return peaks.reduce(pickPeakByDbReducer, peaks[0]);
}

function modeResultFromPeak(mode: string, freqs: number[], dbs: number[], peak: PeakCandidate): ModeResult {
...
function modeResultFromPrimaryPeak(
  modeKey: string,
  freqs: number[],
  dbs: number[],
  primary: PeakCandidate | null,
): ModeResult {
  return primary
...
function scanNearestIndex(values: number[], target: number): number {
  return scanNearestIndexState(values, target).idx;
}

function scanNearestIndexState(values: number[], target: number): { idx: number; bestDist: number } {
  return reduceNearestState(values, target, { idx: 0, bestDist: Infinity });
}

function reduceNearestState(
  values: number[],
  target: number,
  state: { idx: number; bestDist: number },
): { idx: number; bestDist: number } {
...
function updateNearestState(
  ...
  if (dist >= state.bestDist) return state;
  return { idx, bestDist: dist };
}

function findLeftCrossing(freqs: number[], dbs: number[], idx: number, cutoff: number): number | null {
...
function scanLeftIndexForCutoff(dbs: number[], startIdx: number, cutoff: number): number {
  let left = startIdx;
  for (; left > 0 && dbs[left] > cutoff; left -= 1) {}
  return left;
}

function findRightCrossing(freqs: number[], dbs: number[], idx: number, cutoff: number): number | null {
...
function scanRightIndexForCutoff(dbs: number[], startIdx: number, cutoff: number): number {
  let right = startIdx;
  for (; right < dbs.length - 1 && dbs[right] > cutoff; right += 1) {}
  return right;
}

function interpolateDbCrossing(
  freqs: number[],
  dbs: number[],
  cutoff: number,
  i0: number,
...
function interpolateSample(
  sample: { fA: number; fB: number; yA: number; yB: number },
  t: number,
): number {
...
function crossingSamples(freqs: number[], dbs: number[], i0: number, i1: number) {
  const fA = freqs[i0];
  const fB = freqs[i1];
  const yA = dbs[i0];
  const yB = dbs[i1];
...
function finiteCrossingSamples(fA: number, fB: number, yA: number, yB: number) {
  if (!Number.isFinite(fA) || !Number.isFinite(fB) || !Number.isFinite(yA) || !Number.isFinite(yB)) return null;
  return { fA, fB, yA, yB };
}

function interpolationFraction(cutoff: number, yA: number, yB: number): number | null {
  const t = (cutoff - yA) / (yB - yA);
  return Number.isFinite(t) ? t : null;
}

function qFromPeakDbIfAvailable(
  freqs: number[],
  dbs: number[],
  peakValues: { f0: number; peakDb: number } | null,
): number | null {
  if (!peakValues) return null;
  return qFromPeakDb(freqs, dbs, peakValues.f0, peakValues.peakDb);
}

function peakFrequencyAndDb(peak: { freq: number; db: number }): { f0: number; peakDb: number } | null {
  const { freq: f0, db: peakDb } = peak;
  return peakFrequencyAndDbIfFinite(f0, peakDb);
}

function peakFrequencyAndDbIfFinite(
  f0: number,
  peakDb: number,
): { f0: number; peakDb: number } | null {
  if (!Number.isFinite(f0) || !Number.isFinite(peakDb)) return null;
  return peakFrequencyAndDbFromValues(f0 as number, peakDb as number);
}

function peakFrequencyAndDbFromValues(f0: number, peakDb: number): { f0: number; peakDb: number } {
  return { f0, peakDb };
}

function qFromPeakDb(freqs: number[], dbs: number[], f0: number, peakDb: number): number | null {
...
function qFromCrossingsIfAvailable(
  f0: number,
  crossings: { leftF: number; rightF: number } | null,
): number | null {
  if (!crossings) return null;
  return qFromCrossings(f0, crossings.leftF, crossings.rightF);
}

function cutoffDbFromPeak(peakDb: number): number {
  return peakDb - 3;
}

function peakIndexFromFrequency(freqs: number[], f0: number): number {
  return findNearestIndex(freqs, f0);
}

function crossingFrequenciesForCutoff(
  freqs: number[],
  dbs: number[],
  idx: number,
  cutoff: number,
): { leftF: number; rightF: number } | null {
...
function crossingsFromOptionalFrequencies(leftF: number | null, rightF: number | null): {
  leftF: number;
  rightF: number;
} | null {
  if (!Number.isFinite(leftF) || !Number.isFinite(rightF)) return null;
  return { leftF: leftF as number, rightF: rightF as number };
}

function qFromCrossings(f0: number, leftF: number, rightF: number): number | null {
  const bw = Math.max(1e-6, Math.abs(rightF - leftF));
  return qFromBandwidth(f0, bw);
}

function qFromBandwidth(f0: number, bw: number): number | null {
  return qFromValidatedValue(qFromBandwidthValue(f0, bw));
}

function qFromBandwidthValue(f0: number, bw: number): number {
  return f0 / bw;
}

function qFromValidatedValue(q: number): number | null {
