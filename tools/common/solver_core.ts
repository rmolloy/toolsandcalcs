...
(() => {
  ...
  interface SolverParams extends Record<string, number | undefined> {
    model_order: number;
    mass_top: number;
    damping_top: number;
    stiffness_top: number;
    area_top: number;
    mass_back: number;
    damping_back: number;
    stiffness_back: number;
    area_back: number;
    mass_sides: number;
    damping_sides: number;
    area_sides: number;
    mass_air: number;
    damping_air: number;
    area_hole: number;
    ...
    eta_tripole?: number;
    mass_dipole?: number;
    stiffness_dipole?: number;
    damping_dipole?: number;
    area_dipole?: number;
    k_top_dipole?: number;
    alpha_dipole?: number;
    mass_tripole?: number;
    stiffness_tripole?: number;
    damping_tripole?: number;
    area_tripole?: number;
    k_top_tripole?: number;
    alpha_tripole?: number;
  }
  ...
  function sweep(f1 = 0, f2 = 500, step = 0.1): number[] {
    const out: number[] = [];
    for (let f = f1; f <= f2 + 1e-9; f += step) out.push(+f.toFixed(4));
    return out;
  }
  ...
  const inv = (a: Complex): Complex => { const d = a.re * a.re + a.im * a.im || 1e-30; return C(a.re / d, -a.im / d); };
  const div = (a: Complex, b: Complex): Complex => mul(a, inv(b));
  const abs = (a: Complex): number => Math.hypot(a.re, a.im);

  /* determinants */
  function det2(a11: Complex, a12: Complex, a21: Complex, a22: Complex): Complex { return sub(mul(a11, a22), mul(a12, a21)); }
  function det3(a11: Complex, a12: Complex, a13: Complex, a21: Complex, a22: Complex, a23: Complex, a31: Complex, a32: Complex, a33: Complex): Complex {
  ...
  /* shared helpers */
  type Atmosphere = { rho: number; c: number };

  function resolveAtmosphere(p: SolverParams): Atmosphere {
  ...
  function scalePressure(y: Complex, area: number, om: number, rhoOverride?: number): Complex {
  ...
  function kappaFrom(p: SolverParams, atm?: Atmosphere): number {
  ...
  function computeResponse1DOF(p: SolverParams): SolverResponse {
  ...
  function computeResponse2DOF(p: SolverParams): SolverResponse {
    const atm = resolveAtmosphere(p);
    const kappa = kappaFrom(p, atm);
  ...
  function computeResponse3DOF(p: SolverParams): SolverResponse {
    const atm = resolveAtmosphere(p);
    const kappa = kappaFrom(p, atm);
    const omt = Math.sqrt((p.stiffness_top + kappa * p.area_top * p.area_top) / p.mass_top);
  ...
  function computeResponse5DOF(p: SolverParams): SolverResponse {
    return computeExtendedResponse(p, { includeDipole: true, includeTripole: false });
  }

  function computeResponse6DOF(p: SolverParams): SolverResponse {
    return computeExtendedResponse(p, { includeDipole: true, includeTripole: true });
  }

  /* wrapper */
  function computeResponse(p: SolverParams): SolverResponse {
  ...
  type ExtendedOptions = { includeDipole?: boolean; includeTripole?: boolean };
  type DofConfig = {
    id: string;
    traceKey: string;
    mass: number;
  ...
  type Coupling = { source: string; target: string; stiffness: number };
  type ExtendedConfig = { dofs: DofConfig[]; couplings: Coupling[]; driveId: string };

  function computeExtendedResponse(p: SolverParams, { includeDipole = false, includeTripole = false }: ExtendedOptions = {}): SolverResponse {
    const atm = resolveAtmosphere(p);
    const kappa = kappaFrom(p, atm);
  ...
  function buildExtendedSystem(p: SolverParams, kappa: number, { includeDipole, includeTripole }: { includeDipole: boolean; includeTripole: boolean }): ExtendedConfig {
    const eta = (key: keyof SolverParams, fallback: number) => (typeof p[key] === "number" ? p[key] as number : fallback);
  ...
  function solveCoupledSystem(p: SolverParams, atm: Atmosphere, config: ExtendedConfig): SolverResponse {
    ...
    return { total, ...traceSeries } as SolverResponse;
  }

  function buildSystemMatrix(dofs: DofConfig[], couplings: Array<{ i: number; j: number; stiffness: number }>, omega: number): Complex[][] {
    const size = dofs.length;
    const matrix: Complex[][] = Array.from({ length: size }, () => Array.from({ length: size }, () => C(0, 0)));
  ...
  function solveComplexLinearSystem(matrix: Complex[][], rhs: Complex[]): Complex[] {
    ...
    const x: Complex[] = Array(n).fill(C(0, 0));
    for(let i=n-1;i>=0;i--){
      let sum = C(0, 0);
      for(let j=i+1;j<n;j++){
  ...
  if (globalScope) {
    (globalScope as any).SolverCore = SolverCore;
    (globalScope as any).ModelCore = SolverCore;
  }
  if (typeof module !== "undefined" && (module as any).exports) {
    (module as any).exports = SolverCore;
  }
})();
