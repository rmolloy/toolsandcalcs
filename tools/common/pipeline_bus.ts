import type { PipelineContext, PipelineEventName, PipelineHandler } from "./pipeline_events.js";

export type PipelineBus = {
  wire: (event: PipelineEventName, handler: PipelineHandler) => void;
  emit: (event: PipelineEventName, payload: unknown) => Promise<void>;
  context: PipelineContext;
};

type PipelineBusOptions = {
  logPrefix?: string;
};

export function createPipelineBus(options: PipelineBusOptions = {}): PipelineBus {
