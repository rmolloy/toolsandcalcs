...
(() => {
    var _a;
    const $ = globalThis.$;
    const fmt = globalThis.fmt;
    const computeResponse = globalThis.computeResponse;
    ...
    function activeColors() { var _a, _b; return (typeof document !== "undefined" && ((_b = (_a = document.body) === null || _a === void 0 ? void 0 : _a.dataset) === null || _b === void 0 ? void 0 : _b.palette) === "whatif") ? WHATIF_COLORS : DEFAULT_COLORS; }
    ...
    function hexToRgb(hex) {
        const clean = hex === null || hex === void 0 ? void 0 : hex.replace("#", "");
    ...
    const AtmosphereLib = (typeof globalThis !== "undefined" && globalThis.Atmosphere)
        ? globalThis.Atmosphere
        : null;
    if (!AtmosphereLib) {
        throw new Error("Missing Atmosphere library. Ensure tools/common/atmosphere.js is loaded before main.js.");
    }
    const { deriveAtmosphere, formatAtmosphereSummary, formatEffectiveAirMass, FT_PER_M, REFERENCE_RHO } = AtmosphereLib;
    ...
    function isWhatIfPage() {
        var _a, _b;
        return typeof document !== "undefined" && ((_b = (_a = document.body) === null || _a === void 0 ? void 0 : _a.dataset) === null || _b === void 0 ? void 0 : _b.palette) === "whatif";
    }
    ...
    { minDb = -90, minProm = 4, minDistHz = 20 } = {}) {
        ...
        for (let i = 1; i < series.length - 1; i++) {
            ...
            let minL = y, minR = y;
            for (let k = 1; k <= W && i - k >= 0; k++) {
                if (series[i - k].y < minL)
                    minL = series[i - k].y;
            }
            for (let k = 1; k <= W && i + k < series.length; k++) {
                if (series[i + k].y < minR)
                    minR = series[i + k].y;
            }
            const prom = y - Math.max(minL, minR);
            if (prom >= minProm)
                cands.push({ i, f: series[i].x, y, prom });
        }
        // greedy select by height with min distance
        cands.sort((a, b) => b.y - a.y);
        const picked = [];
        for (const p of cands) {
            if (picked.some(q => Math.abs(q.f - p.f) < minDistHz))
                continue;
            picked.push(p);
        }
    ...
    function detectPeaksAdaptive(series, desiredCount, baseOpts) {
        ...
        for (let i = 0; i < attempts.length; i++) {
            ...
            if (result.length >= desiredCount || i === attempts.length - 1)
                return result;
        }
        return [];
    }
    function dominantAt(index, R) {
        ...
        const vals = {
            top: (_b = (_a = R.top[index]) === null || _a === void 0 ? void 0 : _a.y) !== null && _b !== void 0 ? _b : -140,
            back: (_d = (_c = R.back[index]) === null || _c === void 0 ? void 0 : _c.y) !== null && _d !== void 0 ? _d : -140,
            air: (_f = (_e = R.air[index]) === null || _e === void 0 ? void 0 : _e.y) !== null && _f !== void 0 ? _f : -140,
            sides: (_h = (_g = R.sides[index]) === null || _g === void 0 ? void 0 : _g.y) !== null && _h !== void 0 ? _h : -140
        };
    ...
    function buildAnnotations(peaks, whatPeaks, baseResponse, showWhatIf, order) {
        ...
        peaks.forEach((pk, idx) => {
            ...
            const what = showWhatIf ? whatPeaks === null || whatPeaks === void 0 ? void 0 : whatPeaks[idx] : null;
            let text = `${name}<br>${pk.f.toFixed(1)} Hz`;
            if (what)
                text += `<br>What-If ${what.f.toFixed(1)} Hz`;
            annotations.push({
                x: pk.f,
                y: pk.y,
                text,
    ...
    function buildWhatIfPeakList(basePeaks, whatPeaks, order) {
        return basePeaks.map((pk, idx) => {
            ...
            const what = whatPeaks === null || whatPeaks === void 0 ? void 0 : whatPeaks[idx];
            if (!what)
                return `<div class="pl-item">${baseText}</div>`;
    ...
    function readUiInputs() {
        ...
        const modelOrderEl = $("model_order");
        return {
            ...
            model_order: parseInt((_a = modelOrderEl === null || modelOrderEl === void 0 ? void 0 : modelOrderEl.value) !== null && _a !== void 0 ? _a : "4", 10),
            show_labels: Boolean(((_b = $("show_labels")) === null || _b === void 0 ? void 0 : _b.checked))
        };
    }
    ...
    function applyRawValues(raw) {
        Object.entries(raw).forEach(([key, val]) => {
            ...
            if (el.type === "range") {
                const lab = $(`${key}_val`);
                if (lab)
                    lab.textContent = formatSliderValue(key, next);
            }
        });
        render();
    }
    ...
    function hasFitTargets(targets) {
        if (targets.freqs && targets.freqs.some(v => Number.isFinite(v)))
            return true;
    ...
    function evaluateFitDetail(raw, targets) {
        ...
        let cost = 0;
        const freqErrors = [];
        (_a = targets.freqs) === null || _a === void 0 ? void 0 : _a.forEach((target, idx) => {
        ...
        if (Number.isFinite(targets.mass_top))
            cost += sqRelative(raw.mass_top, targets.mass_top);
        if (Number.isFinite(targets.stiffness_top))
            cost += sqRelative(raw.stiffness_top, targets.stiffness_top);
        if (Number.isFinite(targets.mass_back))
            cost += sqRelative(raw.mass_back, targets.mass_back);
        if (Number.isFinite(targets.stiffness_back))
            cost += sqRelative(raw.stiffness_back, targets.stiffness_back);
        if (Number.isFinite(targets.volume_air))
            cost += sqRelative(raw.volume_air, targets.volume_air);
        if (Number.isFinite(targets.area_hole))
            cost += sqRelative(raw.area_hole, targets.area_hole);
    ...
    function runCoordinateDescent(initialRaw, adjustableIds, targets, { maxIter = 80, clampDirection } = {}) {
        if (!adjustableIds.length)
            throw new Error("No adjustable parameters available.");
        let best = { ...initialRaw };
        ...
        for (let iter = 0; iter < maxIter; iter++) {
            ...
            for (const id of adjustableIds) {
                ...
                const tryDelta = (delta) => {
                    if (clampDirection && !clampDirection(id, delta))
                        return null;
                ...
                let next = null;
                if (plus && plus.cost < bestCost)
                    next = plus;
                if (minus && minus.cost < ((next && next.cost) || bestCost))
                    next = minus;
                if (next) {
                    best = next.candidate;
                    bestCost = next.cost;
    ...
    function buildDirectionClamp(options) {
        if (!options || !options.restrictSimple)
            return null;
    ...
    function fitWhatIfParameters(targets, opts = {}) {
        ...
        const stiffnessClamp = (id, delta) => {
            if (id === "stiffness_top" || id === "stiffness_back")
                return delta <= 0;
            return true;
        };
        const clampDirection = (id, delta) => {
            if (!stiffnessClamp(id, delta))
                return false;
            if (restrictClamp && !restrictClamp(id, delta))
                return false;
            return true;
        };
    ...
    function initFitAssistUi() {
        ...
        if (!isModal && !isInline)
            return;
        const titleEl = modal === null || modal === void 0 ? void 0 : modal.querySelector("header h2");
        const submitBtn = form.querySelector("button[type=submit]");
        let mode = ((_a = form.dataset) === null || _a === void 0 ? void 0 : _a.fitMode) || "baseline";
        const setMode = (nextMode) => {
            mode = nextMode;
            if (titleEl)
                titleEl.textContent = mode === "whatif" ? "Solve Targets" : "Fit My Guitar";
            if (submitBtn)
                submitBtn.textContent = mode === "whatif" ? "Compute Recipe" : "Fit My Guitar";
        };
        ...
        baselineBtn === null || baselineBtn === void 0 ? void 0 : baselineBtn.addEventListener("click", () => openPanel("baseline"));
        whatIfBtn === null || whatIfBtn === void 0 ? void 0 : whatIfBtn.addEventListener("click", () => openPanel("whatif"));
        if (isModal) {
            ...
            modal.addEventListener("click", evt => {
                if (evt.target === modal || evt.target.classList.contains("fit-modal__backdrop"))
                    closePanel();
            });
        }
        else if (isInline) {
        ...
        if (resetBtn) {
            resetBtn.addEventListener("click", () => {
                var _a, _b;
                if (mode === "whatif") {
                    if (window.resetWhatIfOverlays)
                        window.resetWhatIfOverlays();
                    else
                        (_a = document.getElementById("btn_reset_whatif")) === null || _a === void 0 ? void 0 : _a.click();
                    status.textContent = "What-If sliders reset.";
                }
                else {
                    (_b = document.getElementById("btn_reset")) === null || _b === void 0 ? void 0 : _b.click();
        ...
        const toggleDisabled = (disabled) => {
            form.querySelectorAll("input,button").forEach(el => {
                if (el.dataset.fitClose !== undefined)
                    return;
        ...
        form.addEventListener("submit", async (evt) => {
            ...
            const restrictSimple = Boolean((_a = $("fit_restrict_simple")) === null || _a === void 0 ? void 0 : _a.checked);
            ...
            try {
                ...
                console.error(err);
                status.textContent = (err === null || err === void 0 ? void 0 : err.message) || (mode === "whatif" ? "Unable to fit What-If." : "Unable to fit your guitar.");
            }
            finally {
    ...
    function unusedRenderNow() {
        ...
        if (base._atm) {
            const meta = $("atm_meta");
            if (meta)
                meta.textContent = formatAtmosphereSummary(base._atm);
    ...
    CONTROL_IDS.forEach(id => {
        ...
        el.addEventListener("input", () => {
            const lab = $(`${id}_val`);
            if (lab && el.type === "range")
                lab.textContent = formatSliderValue(id, parseFloat(el.value));
            if (id === "model_order") {
                const labels = { 1: "1‑DOF", 2: "2‑DOF", 3: "3‑DOF", 4: "4‑DOF" };
                const modelLabel = $("model_label");
                if (modelLabel)
                    modelLabel.textContent = labels[parseInt(el.value, 10)] || "—";
    ...
    (function initVals() {
        CONTROL_IDS.forEach(id => {
            ...
            if (el.type === "range") {
                ...
                const lab = $(`${id}_val`);
                if (lab && !Number.isNaN(v))
                    lab.textContent = formatSliderValue(id, v);
            }
        });
    })();
    /* Show/hide blocks based on model order */
    function setModelVisibility(order) {
        const topBlock = $("blk_top");
        const backBlock = $("blk_back");
        const sidesBlock = $("blk_sides");
        if (topBlock)
            topBlock.style.display = (order >= 2) ? "" : "none";
        if (backBlock)
            backBlock.style.display = (order >= 3) ? "" : "none";
        if (sidesBlock)
            sidesBlock.style.display = (order >= 4) ? "" : "none";
    }
    /* Reset */
    ...
    resetButton === null || resetButton === void 0 ? void 0 : resetButton.addEventListener("click", () => {
        ...
        const modelLabel = $("model_label");
        if (modelLabel)
            modelLabel.textContent = "4‑DOF";
        setModelVisibility(4);
        render();
    });
    /* first render */
    const initialOrderEl = $("model_order");
    setModelVisibility(parseInt((_a = initialOrderEl === null || initialOrderEl === void 0 ? void 0 : initialOrderEl.value) !== null && _a !== void 0 ? _a : "4", 10) || 4);
    render();
