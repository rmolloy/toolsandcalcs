...
type DualSlider = HTMLInputElement;

declare global {
  interface Window {
    resetWhatIfOverlays?: () => void;
    setWhatIfSummary?: (lines: string[] | null) => void;
    render?: () => void;
    formatSliderValue?: (id: string, value: number) => string;
  }
}

(function(){
  const toggle = document.getElementById("whatif_toggle") as HTMLInputElement | null;
  const resetBtn = document.getElementById("btn_reset_whatif") as HTMLButtonElement | null;
  ...
  function resetWhatIf(){
    document.querySelectorAll<DualSlider>(".dual-slider__overlay").forEach(slider=>{
  ...
  function setOverlayValue(slider: DualSlider, value: string | number | null){
    const offValue = parseFloat(slider.getAttribute("data-off-value") || "0");
    const val = value == null ? offValue : parseFloat(value as string);
  ...
  function refreshSliders(){
    document.querySelectorAll<DualSlider>(".dual-slider__base").forEach(base=>{
      updateSliderVisual(base.id);
    });
  }

  function updateSliderVisual(baseId: string){
    const base = document.getElementById(baseId) as DualSlider | null;
    const overlay = document.getElementById(`${baseId}_whatif`) as DualSlider | null;
    const deltaBar = document.getElementById(`${baseId}_whatif_delta`) as HTMLElement | null;
    const label = document.getElementById(`${baseId}_whatif_val`) as HTMLElement | null;
    ...
    const offValue = parseFloat(overlay.getAttribute("data-off-value") || "0");
    ...
    const pct = (val: number)=>`${val.toFixed(4)}%`;
  ...
  const sliders = document.querySelectorAll<DualSlider>(".controls-panel input[type=range]");
  sliders.forEach(slider=>{
    ...
    const overlay = slider.cloneNode(true) as DualSlider;
