...
(() => {
    ...
    function sweep(f1 = 0, f2 = 500, step = 0.1) {
        const out = [];
        for (let f = f1; f <= f2 + 1e-9; f += step)
            out.push(+f.toFixed(4));
        return out;
    }
    /* --------------------- Complex arithmetic --------------------- */
    ...
    function computeResponse1DOF(p) {
        ...
        const F = (_a = p.driving_force) !== null && _a !== void 0 ? _a : 0;
    ...
    function computeResponse3DOF(p) {
        ...
        for (const f of freqs) {
            ...
            //      [      0, backImpedance, -αb],
            //      [-αt,-αb,  airImpedance]]
            const detSystem = det3(topImpedance, C(0, 0), C(-alpha_t, 0), C(0, 0), backImpedance, C(-alpha_b, 0), C(-alpha_t, 0), C(-alpha_b, 0), airImpedance);
            // b = [F, 0, 0]^T
            const detTop = det3(C(F, 0), C(0, 0), C(-alpha_t, 0), C(0, 0), backImpedance, C(-alpha_b, 0), C(0, 0), C(-alpha_b, 0), airImpedance);
            const detBack = det3(topImpedance, C(F, 0), C(-alpha_t, 0), C(0, 0), C(0, 0), C(-alpha_b, 0), C(-alpha_t, 0), C(0, 0), airImpedance);
            const detAir = det3(topImpedance, C(0, 0), C(F, 0), C(0, 0), backImpedance, C(0, 0), C(-alpha_t, 0), C(-alpha_b, 0), C(0, 0));
    ...
    function computeFourDofPressuresAtFrequency(p, frequencyHz) {
        var _a;
        const atm = resolveAtmosphere(p);
        const kappa = kappaFrom(p, atm);
        const omt = Math.sqrt((p.stiffness_top + kappa * p.area_top * p.area_top) / p.mass_top);
    ...
    function buildExtendedSystem(p, kappa, { includeDipole, includeTripole }) {
        ...
        if (includeDipole) {
            ...
            if (alphaDip)
                couplings.push({ source: "dipole", target: "air", stiffness: alphaDip });
        }
        if (includeTripole) {
            ...
            if (alphaTri)
                couplings.push({ source: "tripole", target: "air", stiffness: alphaTri });
        }
        return {
            dofs,
    ...
    function solveCoupledSystem(p, atm, config) {
        ...
        const driveIndex = (_a = idToIndex.get(config.driveId)) !== null && _a !== void 0 ? _a : 0;
        ...
        for (const f of freqs) {
            ...
            rhs[driveIndex] = C((_b = p.driving_force) !== null && _b !== void 0 ? _b : 0, 0);
            ...
            config.dofs.forEach((dof, idx) => {
                ...
                if (dof.area && dof.area > 0) {
                    ...
                    pressure = scl(basePressure, (_a = dof.eta) !== null && _a !== void 0 ? _a : 1);
    ...
    function solveComplexLinearSystem(matrix, rhs) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (let r = i + 1; r < n; r++) {
                ...
                if (Math.abs(factor.re) < 1e-15 && Math.abs(factor.im) < 1e-15)
                    continue;
                for (let c = i; c < n; c++) {
    ...
    if (globalScope) {
        globalScope.SolverCore = SolverCore;
        globalScope.ModelCore = SolverCore;
    }
