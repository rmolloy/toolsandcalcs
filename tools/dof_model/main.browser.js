...
(() => {
  ...
  var DEFAULT_FREQUENCY_END_HZ = 800;
  var DEFAULT_FREQUENCY_START_HZ = 100;
  var DEFAULT_PRESSURE_REFERENCE_PA = 2e-5;
  ...
  // tools/dof_model/dof_plot_callouts.ts
  ...
  var SIMPLE_SOURCE_COLORS = ["var(--orange)", "var(--yellow)", "var(--purple)", "var(--blue)", "var(--green)"];
  var currentParams = { ...DEFAULT_PARAMS };
  var currentOrder = 4;
  var currentTaskMode = "edit";
  var currentSimpleSources = {
    enabled: false,
    sources: []
  };
  ...
  var SIMPLE_SOURCE_DEFAULT_SEMITONE_OFFSET = 3;
  var SIMPLE_SOURCE_FALLBACK_FREQUENCY_HZ = 200;
  function simpleSourceDefaultFrequencyRead() {
    ...
    ].filter((frequency) => Number.isFinite(frequency ?? NaN));
    const highest = candidates.length ? Math.max(...candidates) : SIMPLE_SOURCE_FALLBACK_FREQUENCY_HZ;
  ...
  function updateThumbs(response = lastResponse) {
    ...
    ["air", "top", "back"].forEach((mode) => {
      ...
      calloutTextApply(thumb, calloutTextBuild(MODE_META[mode].label, freq));
  ...
  function renderPlot() {
    ...
    const composedResponse = simpleSourcesActive() ? simpleSourcesCombinedResponseRead(solverParams, xRange[0], xRange[1]) : null;
    const composedWhatIfResponse = simpleSourcesActive() && whatIfParams ? simpleSourcesCombinedResponseRead(adaptParamsToSolver(whatIfParams), xRange[0], xRange[1]) : null;
