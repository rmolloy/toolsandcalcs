...
type OverlaySliderRange = SliderRange & {
  step?: string;
};

export type DofOverlayPresentation = {
  isActive: boolean;
  baseFill: number;
  overlayFill: number;
  start: number;
  end: number;
  deltaBarActive: boolean;
  whatIfActive: boolean;
  overlayValueText: string;
  delta: number | null;
  deltaDigits: number;
};
...
export function sliderFillPercent(slider: SliderRange, value: number) {
...
export function decimalPlacesFromStep(stepValue: number) {
...
export function formatOverlayDisplayValue(value: number, stepValue: number) {
  if (!Number.isFinite(value)) return "--";
  const decimals = Math.min(4, decimalPlacesFromStep(stepValue));
  return value.toFixed(decimals);
}

export function buildDofOverlayPresentation(options: {
  baseSlider: SliderRange;
  overlaySlider: OverlaySliderRange;
  baseValue: number;
  overlayValue: number;
  showWhatIf: boolean;
}): DofOverlayPresentation {
