(function (globalScope) {
  function openConnectedFlexuralSaveModal(args) {
    return new Promise(function (resolve) {
      var modal = buildFlexuralSaveModal(args || {});
      document.body.appendChild(modal);
      attachFlexuralSaveModalBindings(modal, args || {}, function (result) {
        modal.remove();
        resolve(result);
      });
    });
  ...
  function buildFlexuralSaveModal(args) {
    ...
    var subjectOptions = buildFlexuralSubjectOptions(subjects);
    var modal = document.createElement("div");
    modal.className = "save-modal";
    modal.innerHTML = `
      <div class="save-modal__backdrop" data-save-modal-close></div>
      <div class="save-modal__panel" role="dialog" aria-modal="true" aria-label="Save Flexural Rigidity">
        <header>
          <h2>Save to ${escapeFlexuralSaveHtml(args.notebookName || "Notebook")}</h2>
        ...
        <form class="save-modal__form">
          <section class="save-modal__section">
            ...
            <div class="save-modal__new"${subjectMode === "new" ? "" : " hidden"}>
              <label>Display Name<input type="text" name="display_name" placeholder="${escapeFlexuralSaveHtml(args.defaultDisplayName || "Top Plate")}"></label>
            </div>
          </section>
          <section class="save-modal__section">
            <h3>Event</h3>
            <label>Note<textarea name="event_note" rows="4" placeholder="What changed in the panel or brace stack?"></textarea></label>
          </section>
          <section class="save-modal__section">
            <h3>Package</h3>
  ...
  function attachFlexuralSaveModalBindings(modal, args, closeWith) {
    ...
    modal.querySelectorAll('input[name="subject_mode"]').forEach(function (radio) {
      radio.addEventListener("change", function () {
        renderFlexuralSaveModalMode(modal, radio.value);
      });
    });
    ...
    if (filterInput && subjectSelect) {
      filterInput.addEventListener("input", function () {
        renderFlexuralSaveModalFilter(subjectSelect, args.subjects || [], filterInput.value);
      });
      renderFlexuralSaveModalFilter(subjectSelect, args.subjects || [], "");
    }
    ...
    form.addEventListener("submit", function (event) {
      ...
      var subject = subjectMode === "existing"
        ? readExistingFlexuralSubjectSelection(subjectSelect && subjectSelect.value)
        : readNewFlexuralSubjectSelection(modal);

      if (!subject) {
        renderFlexuralSaveModalError(error, "Choose a subject or enter a display name.");
        return;
      }

      renderFlexuralSaveModalError(error, "");
      closeWith({
        subject: subject,
        event: {
    ...
    renderFlexuralSaveModalMode(modal, hasExistingFlexuralSubjects(args.subjects) ? "existing" : "new");
  }

  function hasExistingFlexuralSubjects(subjects) {
    return Array.isArray(subjects) && subjects.length > 0;
  }

  function buildFlexuralSubjectOptions(subjects) {
    return subjects.map(function (subject) {
      return '<option value="' + escapeFlexuralSaveHtml(subject.subjectId || "") + '">' +
        escapeFlexuralSaveHtml(subject.displayName || subject.subjectId || "") +
        "</option>";
    }).join("");
  }

  function readExistingFlexuralSubjectSelection(subjectId) {
    return subjectId ? { subjectId: subjectId } : null;
  }

  function readNewFlexuralSubjectSelection(modal) {
  ...
  function renderFlexuralSaveModalMode(modal, mode) {
    modal.querySelector(".save-modal__existing").hidden = mode !== "existing";
    modal.querySelector(".save-modal__new").hidden = mode !== "new";
  }

  function renderFlexuralSaveModalFilter(subjectSelect, subjects, filterText) {
    ...
    subjectSelect.innerHTML = buildFlexuralSubjectOptions(filtered);
  }

  function renderFlexuralSaveModalError(error, message) {
    if (!error) {
      return;
    }
  ...
  function escapeFlexuralSaveHtml(value) {
    return String(value || "")
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
